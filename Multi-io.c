...
#else
	int epfd = epoll_create(1);//int size 参数已经失效 但必须大于0
//pthread_create();
	struct epoll_event ev;
	ev.events = EPOLLIN;
	ev.data.fd = sockfd;
	
	epoll_ctl(epfd, EPOLL_CTL_ADD, sockfd, &ev);//kv存储，
